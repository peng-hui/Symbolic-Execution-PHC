...
class ParseFunc(object):
    ...
    def __init__(self, script):
        ...
        for i in range(len(Blocks)):
            block.append(Block[i].split('\n'))
        return

    def TravelBlock(nblock):
        ...
        for j in range(len(block)):
            ...
            if block[j].isdigit():
                ...
                self.Parents[node].append(self.Parent(block[j][8:])) # nothing with parent, not deal with it at this moment.
            elif block[j] == 'Expr_Assign':
                self.Expr_Assign(block[j + 1].strip()[5:], block[j + 2].strip()[6:], block[j + 3].strip()[8:])
                j += 3
            elif block[j] == 'Stmt_Jump': 
                self.Stmt_Jump(block[j+1].strip()[8:], node) # there may be a loop
                j += 1
    ...
    def Expr_Assign(var, expr, result):
        ...
        #print(expr, evalue)
        self.Var[int(var[4:vidx])]  = evalue
        self.Var[int(result[4: reidx])]= self.Var[int(var[4:vidx])] # result is equal to var
        return
